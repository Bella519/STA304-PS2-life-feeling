...
Code and data supporting this analysis is available at: https://github.com/Bella519/STA304-PS2-life-feeling 

## Abstract
In this project we aim to find the impact of marital status and family income on Canadian life feeling. After cleaning the GSS 2017 data, we construct the linear regression model, plot figures to show the statistics, discuss the calculation results and analyze the data weakness along with next steps. It is illustrated that life feeling will be higher in families with marital status with companionship and relatively high family income. This is important and useful since people often struggle with whether to get married or pursue a higher income, and this finding can help these.

## Introduction
Nowadays, people’s concept toward family is becoming more and more acute and vague. Meanwhile, more attention has been paid on how family can improve their life feeling in daily lives. While Canada has become more diversified, family related data are also changing rapidly. Therefore, our group used the GSS (General Social Survey) data about families from 2017 to analyze potential factors that may increase households’ life feelings, which are marital status and income in our case. Specifically, we are interested in the relationship between life feeling and marital status, income. Thus, we conduct a series of modelling and analysis in this report as follows. The data section explains various issues related to the data of this project, which is divided into seven small points, including the data itself, the process of data collection and processing, and the variables used in this analysis. After that, we establish a linear regression model while we also discuss the pros and cons of alternative model choice. Linearity of the data, properties of residuals such as normality, homogeneity, spread and independence are also analyzed for our linear regression model. After the model is established and analyzed, the result section makes a systematic summary of statistics, such as estimates and corresponding p-values. It is shown that good marital status with companionship and high income has positive association with life feeling. Then the discussion section illustrates what this finding reveals about real life, the explanation of weakness and a statement of some further work to be done, followed by the last part of reference. By reading this report, people can boost their life feelings with more options with evidence supported.
...
setwd('/Users/amandali/Desktop/project2/2017')
raw_data <- read_csv("AAqOGEML.csv")
...
set.seed(1234)
setwd('/Users/amandali/Desktop/project2/2017')
cleaned_up_data <- read.csv("cleaned up gss.csv")

# survey design

#create frame for 4 variables (no NA,factors)

data <- data.frame(cleaned_up_data$feelings_life,
                   cleaned_up_data$marital_status,
                   cleaned_up_data$income_family)

data <- na.omit(data)

data$cleaned_up_data.marital_status<-as.factor(data$cleaned_up_data.marital_status)

data$cleaned_up_data.income_family <- as.factor(data$cleaned_up_data.income_family)

```

\newline In the data section, basically 7 topics will be discussed:

(1)A brief introduction of the data source--2017 GSS families(source,key feature); (2)Analysis of the questionnaire (content introduction,updates,advantages/disadvantages in various steps); (3)Data collection (methodology and sampling,how they find people take the survey,frame,two types of errors,filtering process,approach,what they do about non-response,the cost of the non-responders,result,a discussion on advantages/disadvantages); (4)Data processing (the cleaning process of the raw data, methodology and approach of the data processing, trade off method of significant differences); (5)Fundamental data summary (target population, population, sample size, target sample size and two categories of data collected by GSS) (6)Discussion of the variables in the group's dataset(how do we construct the variable, a emphasis of the variables; (7)Plots of the raw data

\newline (1)A brief introduction of 2017 GSS families:
 ...
 \item[] For the 2017 questionnaire, GSS focused on families. In the user guide, it is stated that: family experiences the influence of marriage and work track at the same time, in the result of changes occurred actively or passively. As Canada becomes more and more diverse, the composition of families has also changed. GSS 2017 surveys family related questions (e.g: number, characteristics, socio-economic situation, status, composition) and also gives a lot of data answers. In our group's report, we will focus on the impact of marital status and family income on people's life of feeling.

\newline (2)Questionnaire:
 \item[] A large amount of relevant data about the respondents and their family members had been collected in the questionnaire, variables such as Family income and Respondent's children. These questions were developed and revised by statistical Canada's questionnaire design resource centre (QDRC). Here are some updated data collectiong methods related to this analysis:
 \item[] Income in 2017: no oral interview on income would be conducted in 2017, and all income information (including personal and family income) was obtained through tax data link. Those who opposed this approach can choose not to be investigated for tax data. This redefinition of the income survey greatly improves the accuracy of the data and the corresponding rate. But it is undeniable that some respondents still choose not to participate in the income survey.

\newline Frame in 2017: The 2017 frame was redesigned to include "mobile only" households which was also a growing population.
...
\newline (5)fundamental data summary:
\item[] From a statistical point of view, the target population includes all non-institutionalized Canadian residents(living in the 10 provinces of Canada) with the age of 15 years old or older.excluding: 1. Residents of the Yukon, Northwest Territories, and Nunavut 2. Full-time residents of institutions. 
 \item[]In 2017, the target sample size of GSS (the number of respondents expected at the moment) was 20000 which is the frame population, which means that the set of all units covered by the sampling frame is 20000. The sampled population is the population from the survey sample, which is the actual number of respondents, 20602. Further, the target population(the set of all units covered by the objected survey) was around 29225405 from 2016 Cencus, with a calculation of whole Canadian population minus the population under 15 and the population from Yukon, Northwest Territories, and Nunavut (tatsnwt.ca. 2020).
 
 \item[] At the same time, combined with the two goals of GSS, the collected data of GSS include core content and classification variables. The former refers to changes related to the measurement of living conditions / well-being, while the latter describes different population groups (e.g. age, gender, income, education). The former is used to inform specific policy issues, while the latter is used to analyze core data. In this 2017 survey, the core content and classification variables are probably future_children_intention, feelings_life and age, sex, region respectively. At the meantime, the core content used in this analysis report are marital_status, income_family, while classification variable is feelings_life.

\newline (6)discussion of the variables in the group’s dataset:
 \item[] In the following discussion, we will focus on the influence of marital status and family income on household feelings of life. As we can see from the summary, the variables our group tend to use are marital_status, income_family and feelings_life. While the first 2 variables are x explanatory variable(and categorical) Last one, feelings_life is the response variable. All variables are directly extracted from the sorted data set, and there is no combined variable.
 \item[] As for their name, marital_status is in character class, represent the person’s marital status. It is seperated into 5 categorized answer of "single, never married","married","living common-law","divorced" and "seperated"; Income_family is also in character type with six levels of $25,000 to $49,999,$75,000 to $99,999... $125,000 and more; Finally the response variable feelings_life is in numeric type. It's a continuous numeric number between 0 and 10. 0 represents the lowest level of life feelings, while 10 is the highest level of satisfaction with one's own life. The minimum of this data is 0 and the maximum goes to 10, and we can see from the summary, the average life of feelings rate is 8.094 for all Canadian residents which is a quiet high rate actually.
 \item[] There are a few variables similar with the used 3 variables, for example income_respondent is a similar varriable with income_family. However, the three variables used are the ones that best fit the topic we are going to discuss, and their values are very representative, like income_family can represent the income of the family as a whole better. Further, the income value is directly extracted from the tax data, thus these variables have relatively few NA values from raw data set(even if the NA values were all deleted in the cleaning process), which greatly ensures the accuracy of our subsequent data analysis.
```{r,include=FALSE,warning=FALSE,message=FALSE}
#summarize the dataframe:gss
summary(gss)

#to see the unique values of marital_status and income_family from gss
unique(gss$marital_status)
unique(gss$income_family)
```

\newline (7)Plots of the raw data:
 \item[] To plot the related raw data, the first step is to find the raw data name corresponding to the cleaned variables:
marital_status ->marstat, income_family ->famincg2 and feelings_life->slm_01
 \item[] Boxplots are used for all 3 graphs. Since the raw data used numeric data to represent all of the imformation with NA values，boxplot can better show the trend of raw data. In this kind of plot, NA values are basically attributed to outlier. The general trend of the main data can be displayed between Q1 and Q3.
 \item[] Here are the graphs, boxplots are used for all of the plots while the order of the graphs is：
 Figure A: Marital Status of the respondents from Raw Data; Figure B:Total Income of Households from Raw Data; Figure C: Feelings of Life Rate for Respondents
 \item[] Figure A: the marital status is concentrated at about 1 while the outliers around 100 represents the NA values.
  \item[]Figure B: the total income of households, the data shows a basically range from 2-5 while the mean value is around 3.
  \item[]Figure C: the life feelings are basically within 0-15 and a couple of NA counts are also observed.
```{r,ehco=FALSE}
#1
boxplot(raw_data$marstat,main="Figure A: Marital Status of the respondents from Raw Data",xlab="Marital Status",ylab="Count",border="pink",density=10)
#2
boxplot(raw_data$famincg2,main="Figure B: Total Income of Households from Raw Data",xlab="Total Income of Each Household",ylab="Count",border="pink",col="purple",density=10)
#3
boxplot(raw_data$slm_01,main="Figure C: Feelings of Life Rate for Respondents",xlab="Each Respondent's Feeling of Life rate",ylab="Count",border="purple",col="white")
```
...
n=length(data$cleaned_up_data.feelings_life) # the number of observation
...
mysvylm <- svyglm(cleaned_up_data.feelings_life~cleaned_up_data.marital_status+cleaned_up_data.income_family,
                  feeling.design,
                  family = "gaussian") 
...
\newline where $\beta_0$ is an intercept parameter which represents the value of the response variable when all other variables equal to 0. And, $\beta_1$,$\beta_2$,.......,$\beta_10$ are the slope parameters that represent the increment in the response per unit change in independent variables. 

\newline In this situation, we know that the marital status variable and family income are categorical variables(i.e a categorical variable is one that has two or more categories) which include several types, and each type will cause a different life feeling. Hence, there are several $\beta$ for each categorical variable. Also, in this formula, the marital status divorced and family income 100,000 to 124,999 are base categories(all other categories in one variable will be compared to base one) that defines the meaning of intercept parameter. Therefore, for x_{livingcommon-law},x_{married},x_{separated},x_{single, nevermarried} and x_{widowed}, each variable is dummy variable which results in either one or zero and all of them only can exist at most one value 1. Also, x_{125000andmore},x_{25000to29999},x_{50000to74999},x_{75000to99999} and x_{lessthan25000} hold the same rules.

\newline More detaily, 
\newline $\beta_0$ represents the value of people's life feelings when a person's marital status is divorced with family income 100,000 to 124,999.
\newline $\beta_1$ represents the increment of life feeling when the marital status changes from divorced to living common-law. 
\newline $\beta_2$ represents the increment of life feeling when the marital status changes from divorced to married.
\newline $\beta_3$ represents the increment of life feeling when the marital status changes from divorced to separated.
\newline $\beta_4$ represents the increment of life feeling when the marital status changes from divorced to single, never married. 
\newline $\beta_5$ represents the increment of life feeling when the marital status changes from divorced to widowed. 
\newline $\beta_6$ represents the increment of life feeling when the family income changes from 100,000 to 124,999 to 125,000 or more. 
\newline $\beta_7$ represents the increment of life feeling when the family income changes from 100,000 to 124,999 to 25000 to 29999. 
\newline $\beta_8$ represents the increment of life feeling when the family income changes from 100,000 to 124,999 to 50000 to 74999. 
\newline $\beta_9$ represents the increment of life feeling when the family income changes from 100,000 to 124,999 to 75000 to 99999. 
\newline $\beta_10$ represents the increment of life feeling when the family income changes from 100,000 to 124,999 to less than 25000. 


\newline For Alternative model:

\newline The alternative model is the Bayesian statistics linear regression model. Bayesian statistics is another approach in statistics such that it assumes parameters follow some distribution. In linear regression we are trying to model: 

$life\;feeling =  \beta_{0} +  \beta_{1}x_{livingcommon-law} +  \beta_{2}x_{married} -  \beta_{3}x_{separated} -  \beta_{4}x_{single, nevermarried} + \\\qquad \beta_{5}x_{widowed} +  \beta_{6}x_{125000andmore} -  \beta_{7}x_{25000to29999} -  \beta_{8}x_{50000to74999} -  \beta_{9}x_{75000to99999} -  \beta_{10}x_{lessthan25000}$

\newline Notice that it is a similar formula for our original models, where parameters and $x_{i}$ have the same meaning. 
\newline Our parameters of interest are $\beta_{0}$,..., $\beta_{10}$.
\newline In Bayesian statistics, we will choose the prior distribution which is the prior belief of our parameters of interest. Here, we will assume that the prior distribution of $\beta_{i}$ is uniform(-10, 10), since we know the value of life feeling ranges from 0 to 1. Uniform(-10,10) defines equal probability over a given range -10 to 10. 
\newline We want to find the posterior distribution by updating based on data and prior knowledge. 
\newline We will use the brms package in R to solve for our parameter of interest.

\newline There are some advantages of using the alternative model which is the Bayesian approach. Firstly, we can combine prior with data in a principled way, say, we can add our prior belief and as we include more data the prior becomes less influential. Secondly, using Bayesian inference we can recover all the inferences made in frequentist approach as a special case. Finally, this alternative model takes fuller account of the uncertainties related to models and parameter values. In contrast, in our original model we fixed our parameters which will impact our outcome a lot with considerable uncertainty.

\newline On the other hand, there also are several weaknesses of this alternative model. Firstly, the alternative model takes a plenty of time to run since it requires a lot of computations. Secondly, choosing our prior is problematic. To come up with a reasonable prior requires a lot of work and thinking. In this case, we probably give a bad prior in our alternative model, since $\beta_{1}$ to $\beta_{10}$ range from -1 to 1 rather than -10 to 10. Finally, this approach demands knowledge that we have not learned. For instance, probability theory and Markov chain. 
...
\newline This is a plot that shows residuals in four different ways. Residuals represent the error between observed y value and fitted y value.

\newline Linear regression model includes 4 assumptions. The first one is linearity of the data, which means the relationship between response variable and independent variables should be linear. The second one is normality of residuals, which means the residuals errors should be normalized. The third one is homogeneity of residuals variance, which means the variance(spread) of residuals should be constant. The last one is independence of residuals error terms.

\newline For linearity of the data, we should look through the figure 1 "residuals vs fitted" plot. In this plot, there is no fitted pattern shown, this tells us we can assume there exists a linear relationship between response variable(life feeling) and predictor variables(family income and marital status).

\newline For normality of residuals, we should look through the figure 2 "normal Q-Q" plot . In this plot, there are lots of points that do not fall along the reference line(dashed line). Hence the assumption "the normality of residuals" is failed. This reveals that the model is not adequate enough, in which errors are not random. Even though this might influence the validity of statistics like p-value, with a large sample size, this will not result in severe consequences. 

\newline For homogeneity of residuals variance, we should look at the figure 3 "Scale-Location" plot (as same name as spread-location plot). In this plot, the red line is almost horizontal which means there is no pattern that exists, hence we can conclude that the variance of residuals are constant.

\newline In “Figure 4: Residuals vs Leverage” plot", we can observe how the spread of residuals change as leverages change. Based on the cook’s distance, we can detect that there are many leverage points as leverage increases and influential points which are outside the red dotted line. 

\newline For independence of residuals error terms, we should look at the "figure 5: residuals vs ordered observation" plot. By this plot, we can conclude that this assumption is satisfied because the red dashed line is horizontal. Horizontal line shows there is no pattern.
...
dataframe <- data.frame(data$cleaned_up_data.marital_status,
                        data$cleaned_up_data.income_family)


plot.design(x=dataframe,y=data$cleaned_up_data.feelings_life,cex=0.6,
...
mean_feeling = mean(data$cleaned_up_data.feelings_life) # mean of life feelings

sd_feeling = sd(data$cleaned_up_data.feelings_life) # median of standard deviation of life feeling

median_feeling = median(data$cleaned_up_data.feelings_life) # median of life feeling
...
\newline Figure 6  "the average life feeling on each level of each factor" has marital status and family income factors on the x axis, and the value of life feeling as a whole on the y axis. The long horizontal line in the middle of the graph shows the average value of life feeling of our whole data set. There are two vertical lines for independent variables marital status and income family respectively, and the dash on the vertical line indicates the average value of life feeling of people who belong to the corresponding level(category). For instance, the top dash on the left vertical line demonstrates that the average value of life feeling of people who are married  is 8.43.

\newline From the graph "the average life feeling on each level of each factor", we can observe that as income of a family increases, the well being tends to increase, and married people generally have a large value of feeling life as a whole. Here we can find out that the average value of life feeling of our data set is about 8.1. For variable marital status, people who live without a partner/spouse have an average value of life feeling less than average value of life feeling of the whole data set. Specifically,  widowed, divorced, single(never married), and separated have averages below the horizontal line y=8.1. Similarly, the average value of life feeling of people who have total family income less than 50,000 are smaller 8.1.

\newline Figure 7 illustrates model results including estimate value, standard error of estimate, and p value of estimate. In this case, both marital status and family income are categorical variables, so predictor variables are assigned as dummy variables. Therefore, $x_{i}$'s either equal to 0 or 1 where 1 represents the presence of a qualitative attribute, and 0 represents the absence.

\newline We get the formula:

\newline $life\;feeling = 7.9682 + 0.3521x_{livingcommon-law} + 0.5472x_{married} - 0.3639x_{separated} - 0.0588x_{single, nevermarried} + \\\qquad0.2956x_{widowed} + 0.0413x_{125000andmore} - 0.2755x_{25000to29999} - 0.1482x_{50000to74999} - 0.1073x_{75000to99999} - 0.5562x_{lessthan25000}$

\newline $\beta_0$ = 7.9682 represents that if a person's marital status is divorced with family income $100,000 to 124,999, the average value of y life feeling is 7.9682.
\newline $\beta_1$ = 0.3521 means if the marital status is living common-law instead of divorced, then the average of y increases 0.31521. 
\newline $\beta_2$ = 0.5472 means if the marital status is married instead of divorced, then the average of y increases 0.5472.
\newline $\beta_6$ = 0.0413 means if the family income is 125,000 or more instead of 100,000 to 124,999, then the average of y increases 0.0413.
\newline Same with all other attributes $\beta_i$. 

\newline The p-value of $\beta_4$ is 0.25907 which is larger than 0.05. Thus we have no evidence against the null hypothesis that \newline $\beta_4$ = 0. Also, The p-value of $\beta_6$ is 0.24363, it is larger than 0.05. Thus we fail to reject the null hypothesis that 
\newline $\beta_6$ = 0. The p-values for $\beta_i$ are close to 0 where i is not 4 or 6, and they are less than 0.05. Thus, we have strong evidence against the null hypothesis that $\beta_i$ = 0.

\newline From figure 8, we can find that R squared is 0.0584, which means 5.84% of variation of y feeling life is explained by our regression line. The statistic p value for our model is 0, which means we have strong evidence against our initial hypothesis that $\beta_i$ = 0. Overall the performance is not very well.

\newline In figure 8, we can also obtain the mean of our data set which is 8.0943 with standard derivation 1.7654.


## Discussion

\newline The first point illustrated in the results is that individuals with companionship tend to have a higher life feeling compared with other single cases. Christopher Munsey in American psychological association states that marriage, wealth and kids do bring happiness. Besides, the second point is that people with higher income usually have a better life feeling. This is congenial with common sense and has been validated by a few researches. For example, there are two possible mechanisms among many, that can plausibly explain the positive effect of income on well-being, adaption and social comparison mechanisms. (Luis Angeles, 2010) 

\newline With these two findings in our model, people can pursue their well-being with more feasible options and more determinedly. For example, single people can try to find someone to know and companion each other. Working people can be more motivated to work hard to get a decent income since higher income usually leads to better well-being as shown in the model. Parents can teach their childs to acquire higher level education to prevent suffering from low income in the future. (statistics Canada, 2017)

\newline This model also provides some caveats for people in their life. For married couples, divorcing or separating largely decreases well-being, so it is important to maintain a happy relationship for themselves and their couples. For working people, earning an income below 25,000 results in a much lower life feeling compared with those who earn an income higher than 25,000. Thus, the finding in the model advises against not working and suffering from too low income.
...
Short, Doug. “Happiness Revisited: A Household Income of $75K?” Advisorperspectives.com, www.advisorperspectives.com/dshort/commentaries/2016/10/21/happiness-revisited-a-household-income-of-75k. 

Lucas, Richard, et al. Reexamining Adaptation and the Set Point Model of ... www.apa.org/pubs/journals/releases/psp-843527.pdf. 

“gss_cleaning.” University of Toronto Quercus , Oct. 2020, q.utoronto.ca. 

R Get All Categories in Column, Stack Overflow, 2 Sept. 2020, stackoverflow.com/questions/46017812/r-get-all-categories-in-column. 

“R Boxplot() to Create Box Plot (With Numerous Examples).” DataMentor, 11 Apr. 2018, www.datamentor.io/r-programming/box-plot/. 

Editor, Minitab Blog. “How Important Are Normal Residuals in Regression Analysis?” Minitab Blog, blog.minitab.com/blog/adventures-in-statistics-2/how-important-are-normal-residuals-in-regression-analysis. 

 Angeles, Luis. “Adaptation or Social Comparison? The e§Ects of Income on Happiness.”  , 21 Jan. 2020, www.gla.ac.uk/media/Media_110443_smxx.pdf. 
...
White, James M. “Marital Status and Well-Being in Canada: An Analysis of Age Group Variations - JAMES M. WHITE, 1992.” SAGE Journals, 1 Sept. 1992, journals.sagepub.com/doi/10.1177/019251392013003008. 

“Age &amp; Sex and Type of Dwellings – 2016 Census –.” Newscast, Government of Northwest Territories , 3 May 2017, www.statsnwt.ca/census/2016/Age-Sex_final.pdf. 

“The Effect of Income on Happiness.” The World Counts, www.theworldcounts.com/happiness/the-effect-of-income-on-happiness. 

“This Census in Brief Article Compares the Earnings of Canadians by Their Highest Level of Educational Attainment, I.e., a High School Diploma, an Apprenticeship Certificate, a College Diploma or a Bachelor's Degree. It Also Examines How These Earnings Vary across the Provinces and Territories and Highlights How the Earnings of Canadians with Selected Educational Qualifications Changed over the Decade Preceding the 2016 Census.” Census in Brief: Does Education Pay? A Comparison of Earnings by Level of Education in Canada and Its Provinces and Territories, Government of Canada, Statistics Canada, 29 Nov. 2017, www12.statcan.gc.ca/census-recensement/2016/as-sa/98-200-x/2016024/98-200-x2016024-eng.cfm. 
